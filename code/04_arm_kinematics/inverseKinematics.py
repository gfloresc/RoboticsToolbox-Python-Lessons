...
def build_sequence(tx, ty):
    """Build full animation trajectory for a given IK target."""
    q_sol = ik(tx, ty) if ik(tx, ty) is not None else q_home.copy()
    segs = [
        (eased(q_home, q_A,  70),
        ...
        (eased(q_B,   q_sol, 90),
         ...
         f'Analytical IK → target ({tx:.2f}, {ty:.2f})',
         (tx, ty)),
    ]
...
# IK target marker
tgt_marker, = ax_robot.plot([1.5], [1.0], '+',
                              color=CORAL, ms=18, mew=2, zorder=5)
...
def update_visuals(q, cur_tgt=None):
    ...
    if cur_tgt is not None:
        ...
        tgt_label.set_text(f'target ({cur_tgt[0]:.2f}, {cur_tgt[1]:.2f})')
...
def on_target_slider(_):
    """Rebuild sequence with new target and restart animation."""
    global traj, phases, msgs, tgts
    tx, ty = sl_tx.val, sl_ty.val
    traj, phases, msgs, tgts = build_sequence(tx, ty)
    tgt_marker.set_data([tx], [ty])
    tgt_label.set_text(f'target ({tx:.2f}, {ty:.2f})')
